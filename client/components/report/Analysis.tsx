...
function getClusterNum(clusters: Result['clusters']): Record<number, number> {
  const array = clusters.map(c => c.level)
...
export function Analysis({clusters, commentsNum, argumentsNum, config}: ReportProps) {

  const [selectedData, setSelectedData] = useState<{ title: string, body: string } | null>(null)
  const clusterNum = getClusterNum(clusters)
        ...
        <Tooltip
          ...
          <VStack gap={0} w={'200px'}>
            ...
            <Text className={'headingColor'} fontSize={'3xl'} fontWeight={'bold'} lineHeight={1}
              mb={1}>{commentsNum.toLocaleString()}</Text>
          ...
          <TimelineRoot size={'lg'}>
            {config.plan.map(p => (
              <TimelineItem key={p.step}>
                ...
                {p.step === 'extraction' && (
                  <TimelineContent>
                    <TimelineTitle fontWeight={'bold'}>抽出 ({config.extraction.model})</TimelineTitle>
                    <TimelineDescription>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `抽出 - ${p.step}`,
                        body: config.extraction.source_code
                      })}>ソースコード</Button>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `抽出 - ${p.step}`,
                        body: config.extraction.prompt
                      })}>プロンプト</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'embedding' && (
                  <TimelineContent>
                    <TimelineTitle fontWeight={'bold'}>埋め込み ({config.embedding.model})</TimelineTitle>
                    <TimelineDescription>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `埋め込み - ${p.step}`,
                        body: config.embedding.source_code
                      })}>ソースコード</Button>
                    </HStack>
                  </TimelineContent>
                )}
                  ...
                  <TimelineContent>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        ...
                        body: config.hierarchical_clustering.source_code
                      })}>ソースコード</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'hierarchical_initial_labelling' && (
                  <TimelineContent>
                    <TimelineTitle fontWeight={'bold'}>初期ラベリング
                      ({config.hierarchical_initial_labelling.model})</TimelineTitle>
                    <TimelineDescription>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        ...
                        body: config.hierarchical_initial_labelling.source_code
                      })}>ソースコード</Button>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `初期ラベリング - ${p.step}`,
                        body: config.hierarchical_initial_labelling.prompt
                      })}>プロンプト</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'hierarchical_merge_labelling' && (
                  <TimelineContent>
                    <TimelineTitle fontWeight={'bold'}>統合ラベリング
                      ({config.hierarchical_merge_labelling.model})</TimelineTitle>
                    <TimelineDescription>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        ...
                        body: config.hierarchical_merge_labelling.source_code
                      })}>ソースコード</Button>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `統合ラベリング - ${p.step}`,
                        body: config.hierarchical_merge_labelling.prompt
                      })}>プロンプト</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'hierarchical_overview' && (
                  <TimelineContent>
                    <TimelineTitle fontWeight={'bold'}>要約 ({config.hierarchical_overview.model})</TimelineTitle>
                    <TimelineDescription>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `要約 - ${p.step}`,
                        body: config.hierarchical_overview.source_code
                      })}>ソースコード</Button>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `要約 - ${p.step}`,
                        body: config.hierarchical_overview.prompt
                      })}>プロンプト</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'hierarchical_aggregation' && (
                  <TimelineContent>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `出力 - ${p.step}`,
                        body: config.hierarchical_aggregation.source_code
                      })}>ソースコード</Button>
                    </HStack>
                  </TimelineContent>
                )}
                {p.step === 'hierarchical_visualization' && (
                  <TimelineContent>
                    ...
                    <HStack>
                      <Button variant={'outline'} size={'xs'} onClick={() => setSelectedData({
                        title: `表示 - ${p.step}`,
                        body: config.hierarchical_visualization.source_code
                      })}>ソースコード</Button>
                    </HStack>
                  </TimelineContent>
                )}
